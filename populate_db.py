...
for t, mean_v, std_v in traits:
    c.execute('INSERT INTO prs_traits (trait, mean_score, std_score) VALUES (?, ?, ?)', (t, mean_v, std_v))
...
for rsid, trait, allele, weight in weights:
    c.execute('INSERT INTO prs_weights (rsid, trait, risk_allele, effect_weight) VALUES (?, ?, ?, ?)', (rsid, trait, allele, weight))

conn.commit()
conn.close()
print('[✔] genomics.db recreated successfully matching runner.py schema.')
